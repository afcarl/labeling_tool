...
def zeros( dims):
	if len(dims) == 1:
		return [([0] * dims[0])]
	return Mat( ( zeros(dims[1:]) * dims[0]) )
...
def linalg(): pass
def __svd(a):
	u,w,vt = libs.svd.svd(a)
	return array(u), array(w), array(vt).tr()
linalg.svd = __svd
    ...
    def __getitem__(self, a):
    	if isinstance(a, tuple):
    		...
    		for item in a:
    			r = r[item]
    		return r
    	return Table.__getitem__(self,a)
...
if __name__ == "__main__":
	...
	list_a = [[1,0,3],[0,2,5],[1,0,1]]
	"""
	np_a = np.array(list_a)
	print np_a
	np_u, np_w, np_vt =  np.linalg.svd(np_a)
	print "---"
	print np_u
	print np_w
	print np_vt
	...
	print vt
	print "---"
	diag = array([[w[0],0,0],[0,w[1],0],[0,0,w[2]]])
	#print u.dot(diag).dot(vt)
	print diag
	diag = diag+diag
	print diag
