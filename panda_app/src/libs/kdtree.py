...
class KDTreeNeighbours():
    ...
    """
    def __init__(self, query_point, t, markSeen = False):
    ...
    def add(self, node):
        if(self.markSeen and node.seen):
            return
        point = node.point
        ...
        for i, e in enumerate(self.current_best):
            ...
            if e[1] > sd:
                self.current_best.insert(i, [point, sd, node])
                self.calculate_largest()
                return
        # append it to the end otherwise
        self.current_best.append([point, sd, node])
        self.calculate_largest()
    
    def get_best(self):
        best_points = [element[0] for element in self.current_best[:self.t]]
        best_nodes = [element[2] for element in self.current_best[:self.t]]
...
class KDTree():
    ...
    def query(self, query_point, t=1, markSeen = False):
        ...
        if self.root_node != None:
            neighbours = KDTreeNeighbours(query_point, t, markSeen)
